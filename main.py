...
class MedicineReminderApp:
    ...
    def refresh_reminders(self):
        ...
        if self.current_user_filter:
            reminders = self.db_manager.get_reminders_for_date_filtered_by_weekday(self.current_date, self.current_user_filter)
        else:
            reminders = self.db_manager.get_reminders_for_date_filtered_by_weekday(self.current_date)
    ...
    def show_family(self):
        """Show family page"""
        FamilyDialog(self.root, self.db_manager, self.refresh_reminders)
    ...
    def show_history(self):
        """Open history dialog"""
        HistoryDialog(self.root, self.db_manager)
...
class AddReminderDialog:
    ...
    def create_field(self, parent, label_text, field_name, placeholder):
        ...
        if field_name == "patient":
            ...
            var = tk.StringVar(value=user_names[0] if user_names else "Aditya")
...
class FamilyDialog:
    ...
    def __init__(self, parent, db_manager: DatabaseManager, on_change_callback=None):
        self.db_manager = db_manager
        ...
        entry = tk.Entry(main, textvariable=name_var, font=('Arial', 11), relief=tk.FLAT, bd=1, bg='white')
    ...
    def refresh_list(self):
        ...
        users = self.db_manager.get_users()
        for u in users:
            label = f"{u['name']}" + (" (You)" if u['is_self'] else "")
...
class HistoryDialog:
    ...
    def refresh_table(self):
        ...
        users = {u['name']: u['id'] for u in self.db_manager.get_users()}
